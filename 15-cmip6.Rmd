...
extracted <- extract_cmip6_data(stations = station, download_path = "cmip6_downloaded/52_6_50_8")
```
...
```{r, echo = FALSE}
kable(head(extracted$`ssp126_AWI-CM-1-1-MR`), align = c("c")) %>%
...
```{r}
kable(head(change_scenarios), align = c("c")) %>%
  kable_styling("striped", 
                position = "center", 
                font_size = 14) %>% 
