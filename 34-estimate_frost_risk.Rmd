...
Analyzing the risk of spring frost events in temperate fruit trees is essential but provides limited insights for planning future adaptation strategies. A comprehensive assessment must estimate the probability and severity of future frost events while considering the impacts of climate change.

Spring frost risk is influenced by temperature thresholds affecting plant tissues and the temperature-driven progression of tree phenology. Sensitivity to frost increases as trees develop, while the timing of development varies depending on species, cultivar, and location.

A reliable frost risk analysis for future scenarios requires robust temperature projections and an accurate modeling framework for predicting bloom dates. Recent advancements, including future temperature scenarios and the PhenoFlex model, contribute to meeting these requirements.

Past temperature data from Campus Klein-Altendorf is combined with simulated conditions from experimental models to evaluate their relevance for projected future climates. The following plot illustrates this approach:
...
This convex-hull plot illustrates the temperature coverage in CKA from 1958 to 2019, along with all experimental seasons analyzed in the *Experimentally Enhanced PLS* chapter. To integrate historical and experimental data into the phenology modeling approach, records from the same species and cultivar (pear cultivar ‘Conference’) are required. Historical full bloom records for this cultivar are available from 1958 to 2019. However, to minimize potential distortions caused by extreme warming treatments, the experimental data used for the convex-hull plot may need to be refined by excluding unusually warm seasons. The following plot presents an updated version without these extreme conditions:
...
The coverage is not entirely complete, as data for October and November is currently lacking. Filling this gap is challenging since warmer conditions were not included in the experiments during these months.

The analysis will proceed with the available data, with the expectation that future identification of ‘normal seasons’ in the experimental dataset will enhance the validity of the modeling domain.

Next, the dormancy modeling framework `PhenoFlex` will be implemented by integrating historical and experimental records. The following code chunk imports and formats the data to enable model calibration using these records.
...
The fitting procedure for the `PhenoFlex` model can now be applied, following the approach from *The PhenoFlex model* chapter. Wide parameter ranges, especially for `yc` and `zc`, are initially used to allow the model to determine the best estimates. The fitted parameters and predicted bloom dates are then saved in the `data` folder.
...
The results in the table indicate a good preliminary performance of the model in predicting bloom dates for pear trees under future conditions. For a more robust analysis, multiple `PhenoFlex` fittings should be performed to achieve lower `RMSEP` values. Additionally, limiting the number of experimental seasons could help reduce the influence of extreme values and narrow the range of observed conditions in the warmer part of the model validity domain. Further errors may arise from combining datasets obtained through different methodologies, such as mature orchard trees versus young potted trees and variations in observers across experiments. The fitting outcomes are examined below, with labels highlighting seasons where the prediction error exceeds 10 days.
...
If the validation approach is considered satisfactory, the fitted parameters from previous model runs can be used to predict bloom dates under future scenarios. This requires utilizing the data generated in the chapter *Future Temperature Scenarios*, which has already been loaded into `R` for generating the convex-hull plots.

The function `PhenoFlex_GDHwrapper()` will be applied to estimate bloom dates for future conditions. The initial steps in the *for* loop format the data appropriately by deriving hourly temperature records using `stack_hourly_temps()` and defining the seasons with `genSeasonList()`.
...
![](images/future_pheno_pear.png){width="90%"}
